		...
		Create = function()
			...
			local function is_gear(key)
				...
				return key == 'main' or key == 'sub' or key == 'range' or key == 'ranged' or key == 'ammo' or
							 key == 'head' or key == 'neck' or key == 'lear' or key == 'ear1' or key == 'rear' or key == 'ear2' or
							 key == 'body' or key == 'hands' or key == 'lring' or key == 'ring1' or key == 'rring' or key == 'ring2' or
...
--				--add_to_chat(122,'condition = '..tostring(key)..' : '..tostring(value))
	...
	self_command = function(cmd)
		spell = {}
		spell.action_type = 'SelfCommand' 
		spell.arg = split(cmd, ' ')
		spell.english = spell.arg[1]
		rules(spell)
	end
...
setSC = function(sc, target)
    --add_to_chat(122, text)
	if sc == 288 then --light
		LastSC = { Fire = true, Wind = true, Lightning = true, Light = true, target = target, duration = 8}
		add_to_chat(122,'~ ý-ý Light ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 289 then --darkness
		LastSC = { Earth = true, Water = true, Ice = true, Dark = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Darkness ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 290 then --grav
		LastSC = { Earth = true, Dark = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Grav ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 292 then --distortion
		LastSC = { Water = true, Ice = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Induration ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 293 then --fusion
		LastSC = { Fire = true, Light = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Fusion ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 291 then --frag
		LastSC = { Wind = true, Lightning = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Frag ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 294 then --comp
		LastSC = { Dark = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý compression ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 299 then --scis
		LastSC = { Earth = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý scission ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 297 then --reverb
		LastSC = { Water = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Reverb ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 296 then --indur
		LastSC = { Ice = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý Indu ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 298 then --transfix
		LastSC = { Light = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý trans ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 295 then --liqui
		LastSC = { Fire = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý liqui ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 300 then --det
		LastSC = { Wind = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý det ~')
		MBChance = coroutine.schedule(SCDone, .1)
	elseif sc == 301 then --impact
		LastSC = { Lightning = true, target = target, duration = 8 }
		add_to_chat(122,'~ ý-ý impact ~')
		MBChance = coroutine.schedule(SCDone, .1)
	end
end
...
function SCDone()
	LastSC.duration = LastSC.duration or 0
	LastSC.duration = LastSC.duration -.1
	if LastSC.duration <= 0 then
		LastSC = {}
		add_to_chat(122,'~ ý-ý done ~')
	else
		MBChance = coroutine.schedule(SCDone, .1)
	end
end